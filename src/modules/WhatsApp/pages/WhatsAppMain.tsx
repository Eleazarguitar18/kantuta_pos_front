...
const WhatsAppMain = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Esperamos unos segundos para que el backend limpie la sesión y prepare el nuevo QR
      setTimeout(() => {
        checkConnection();
      }, 3000);
    } catch (err: any) {
      setError("Error al cerrar sesión o reiniciar WhatsApp.");
      console.error(err);
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md mx-auto">
        <ComponentCard title="Iniciar Sesión de WhatsApp">
          ...
          <div className="mt-6 flex flex-col gap-4 sm:flex-row">
            <Button
              className="w-full sm:w-1/2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold"
